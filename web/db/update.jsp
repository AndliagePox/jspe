    ...
    if (request.getMethod().equals("POST")) {
        ...
        if (errorMsg.equals("")) {
            ...
            try {
                ...
                rs.next();
                if (rs.getInt(1) != 0) {
                    db.executeUpdate("update product set name='" + name + "', madeTime='" + date + "', price=" + price + " where num=" + num);
                    rs = db.executeSelect("select count(*) from product where num=" + num + " and price=" + price + " and name='" + name + "'");
                    rs.next();
                    if (rs.getInt(1) == 1) {
                        out.println("<script>alert('修改数据成功！');</script>");
                    } else {
